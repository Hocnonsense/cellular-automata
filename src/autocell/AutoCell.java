...
public class AutoCell {
	...
	/** transfer the control system to operate frame */
	public JPanel controlGOL() {
		...
		JButton[] jb = new JButton[str.length]; for (int i : new int[]{0,1,2,3}) {		// described by str[]
			jb[i] = new JButton(str[i]);jb[i].addActionListener(new ActionListener() {
				@Override public void actionPerformed(ActionEvent e) {
					setJButtonColor(jb[i], Utils.testFef(str[i], true));
			}}); setJButtonColor(jb[i], Utils.testFef(str[i], false));
			jp.add(jb[i]);
		}
		
		/** GameOfLife */
		JButton jb5=new JButton("play"); jb5.addActionListener(new ActionListener() {	//the game play or palse
			@Override public void actionPerformed(ActionEvent e) {setJButtonColor(jb5, life.play(true)); }
		}); setJButtonColor(jb5, life.play(false));
		
		/** load or save data */
		JButton jb6=new JButton("save as"); JComboBox<?> jcb = new JComboBox<Object>(getFiles());
		...
		JButton jb7=new JButton("delete"); jb7.addActionListener(new ActionListener() {
			@SuppressWarnings({ "unchecked", "rawtypes" })@Override public void actionPerformed(ActionEvent e) {
	...
	/** reflect the boolean charactor by color */
